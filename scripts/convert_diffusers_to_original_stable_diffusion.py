# copied from https://github.com/huggingface/diffusers/blob/main/scripts/convert_diffusers_to_original_stable_diffusion.py
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rectified_flow",
        action="store_true",
        help="Add rectified_flow flag to the checkpoint.",
    )
    parser.add_argument(
        "--add_flag_only",
        action="store_true",
        help="Only add flags to an already converted checkpoint.",
    )
    ...
    if args.add_flag_only:
        print(f"Loading existing checkpoint from {args.model_path}...")
        ...
        if args.rectified_flow:
            state_dict["rectified_flow"] = torch.tensor([])
        ...
        if args.use_safetensors:
            ...
            torch.save({"state_dict": state_dict}, args.checkpoint_path)

        print(f"Saved updated checkpoint to {args.checkpoint_path}")
        exit(0)
